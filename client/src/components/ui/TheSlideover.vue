...
<template>
  <HuiTransitionRoot as="template" :show="isOpen">
    <HuiDialog as="aside" :class="['fixed inset-0 z-40 flex']" @close="$emit('close')">
      <HuiTransitionChild
        as="template"
        v-if="!hideOverlay"
        ...
        leave-to="opacity-0"
      >
        <HuiDialogOverlay class="fixed inset-0 bg-gray-500/75 dark:bg-gray-800/75" />
      </HuiTransitionChild>
      ...
      >
        <div class="relative flex w-full max-w-xs flex-1 flex-col focus:outline-none">
          <HuiTransitionChild
            as="template"
